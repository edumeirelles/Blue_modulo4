...
namespace Consultorio.Areas.Identity.Pages.Account
{
    ...
    public class LoginWith2faModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            if (user == null)
            {
                throw new InvalidOperationException($"Não foi possível carregar a autenticação de dois fatores.");
            }
        ...
        public async Task<IActionResult> OnPostAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            {
                _logger.LogWarning("Conta de usuário com ID '{UserId}' bloqueada.", user.Id);
                return RedirectToPage("./Lockout");
            }
            else
            {
                _logger.LogWarning("Código de autenticação inválido para o usuário com ID '{UserId}'.", user.Id);
